...
void loadFromPreferences()
{
  ...
  for (int i = 0; i < 8; i++)
  {
    num_leds[i] = preferences.getInt(("num_leds" + String(i)).c_str(), 0);
  }
...
void updateToPreferences()
{
  ...
  for (int i = 0; i < 8; i++)
  {
    preferences.putInt(("num_leds" + String(i)).c_str(), num_leds[i]);
  }
...
String replaceInHtml(String html)
{
  ...
  for (int i = 0; i < 8; i++)
  {
    html.replace("{{num_leds" + String(i) + "}}", String(num_leds[i]));
  }
...
void updateFromServer()
{
  ...
  for (int i = 0; i < 8; i++)
  {
    num_leds[i] = server.arg("num_leds" + String(i)).toInt();
  }
...
void buttonAndLedUpdate()
{
  ...
  if (debouncer.rose())
  {
    ...
    if (debouncer.previousDuration() < 2000)
    { // button released
...
void ledSetup()
{
  ...
  if (led_type == "WS2815")
  {
    ...
    if (0 < num_outputs)
    {
      FastLED.addLeds<WS2812B, DATA_PIN_0>(leds, fastled_index[0], num_leds[0]);
    }
    if (1 < num_outputs)
    {
      FastLED.addLeds<WS2812B, DATA_PIN_1>(leds, fastled_index[1], num_leds[1]);
    }
    if (2 < num_outputs)
    {
      FastLED.addLeds<WS2812B, DATA_PIN_2>(leds, fastled_index[2], num_leds[2]);
    }
    if (3 < num_outputs)
    {
      FastLED.addLeds<WS2812B, DATA_PIN_3>(leds, fastled_index[3], num_leds[3]);
    }
    if (4 < num_outputs)
    {
      FastLED.addLeds<WS2812B, DATA_PIN_4>(leds, fastled_index[4], num_leds[4]);
    }
    if (5 < num_outputs)
    {
      FastLED.addLeds<WS2812B, DATA_PIN_5>(leds, fastled_index[5], num_leds[5]);
    }
    if (6 < num_outputs)
    {
      FastLED.addLeds<WS2812B, DATA_PIN_6>(leds, fastled_index[6], num_leds[6]);
    }
    if (7 < num_outputs)
    {
      FastLED.addLeds<WS2812B, DATA_PIN_7>(leds, fastled_index[7], num_leds[7]);
    }
  }
...
void setup()
{
  ...
  else if (network_type == "ACCESS_POINT")
  {
    ...
    if (!ap_auto)
    {
      WiFi.softAPConfig(ipFromString(ap_ip), ipFromString(ap_gw), ipFromString(ap_sn));
    }
  ...
  else if (network_type == "WIFI")
  {
    ...
    if (!wifi_auto)
    {
      WiFi.config(ipFromString(wifi_ip), ipFromString(wifi_gw), ipFromString(wifi_sn));
    }
    Serial.print("WiFi connected, IP = ");
  ...
  if (use_artsync)
  {
    artnetEth.subscribeArtsync(artsyncCallback);
  }
}
...
void loop()
{
  ...
  {
    artnetEth.parse();
    if (universeMask == expectedUniverseMask || artsync_flag)
    {
      break;
    }
    if (millis() - currentMillis >= frame_time)
    {
      break;
    }
  }
